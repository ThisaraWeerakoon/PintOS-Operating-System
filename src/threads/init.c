...
int pintos_init(void)
{
  ...
  else
  {
    ...
    while (true)
    {
      ...
      while (1)
      {
        char character = input_getc(); // buffer for inputs
        if (character == '\n' || character == '\r')
        {
          input_size[pos] = '\0';
          break;
      ...
      char *pointer;
      char *data = strtok_r(input_size, " ", &pointer);
      if (data != NULL)
      {
        // if strings are equal
        if (strcmp(data, "shutdown") == 0)
        {
          printf("Shutdown PintOS and exit emulator\n");
          shutdown_power_off();
          shutdown();
        }
        // Name of the user
        else if (strcmp(data, "whoami") == 0)
        {
          printf("Rathnayaka W.T - 210536K\n");
        }

        // thread priority of the current thread
        else if (strcmp(data, "priority") == 0)
        {
          int current_priority = thread_get_priority();
          printf("Priority of current thread: %d\n", current_priority);
        }

        // the amount of RAM available for the OS
        else if (strcmp(data, "ram") == 0)
        {
          size_t ram_total = init_ram_pages * PGSIZE / 1024;
          printf("Pintos %'" PRIu32 " kB RAM...\n", ram_total); // init_ram_pages is defined in threads/init.h
        }

        // thread statistics
        else if (strcmp(data, "thread") == 0)
        {
          thread_print_stats();
        }

        // the number of seconds passed since Unix epoch
        else if (strcmp(data, "time") == 0)
        {
          time_t current_time = (time_t)timer_ticks() / TIMER_FREQ;
          printf("number of seconds passed since Unix epoch: %lld seconds\n", (long long)current_time);
        }
